...
/*   Updated: 2024/04/02 15:37:17 by wouhliss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_floorceil(t_game *game, const int w, const int xx)
{
	...
	for (int y = 0; y < HEIGHT; ++y)
	{
		...
		for (int x = xx; x < w; ++x)
		{
			...
			if (is_floor)
			{
				color = ((unsigned int *)game->textures[0].addr)[game->textures[0].s + 64 * t.y
					+ t.x];
			...
			{
				color = ((unsigned int *)game->textures[1].addr)[game->textures[1].s + 64 * t.y
					+ t.x];
				color = (color >> 1) & 8355711;
				my_mlx_pixel_put(&game->screen, x, y, color);
			}
