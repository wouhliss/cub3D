...
/*   Updated: 2024/04/02 13:04:55 by wouhliss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	ft_door(const t_game *game, t_render *render)
{
	...
	render->dhit = 1;
	render->dside_dist.x = render->side_dist.x + render->delta_dist.x / 2;
	render->dside_dist.y = render->side_dist.y + render->delta_dist.y / 2;
	render->dside = render->side;
	if (render->dside > 0)
		render->dperp_dist = render->dside_dist.y - render->delta_dist.y;
	else
		render->dperp_dist = render->dside_dist.x - render->delta_dist.x;
		...
		while (++r.pixel.y < HEIGHT)
		{
			...
			if (r.pixel.y < r.draw.x)
				my_mlx_pixel_put(&game->screen, r.pixel.x, r.pixel.y,
					game->map.c_color);
			else if (r.pixel.y >= r.draw.x && r.pixel.y <= r.draw.y)
